class UserlessRole < ActiveRecord::Base
  ...
  def self.staff_roles
    where(status: "Active", role: ["Team Manager", "Trainer", "Coach", "Athletic Director", "School Manager"])
  end
  
  def self.unique_staff_roles
    array =[]
    self.staff_roles.pluck(:first_name, :last_name).uniq.each do |first, last|
      array << where(status: "Active", role: ["Team Manager", "Trainer", "Coach", "Athletic Director", "School Manager"], first_name: first, last_name: last).last.id
    end
