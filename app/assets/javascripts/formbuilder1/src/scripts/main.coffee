...
class ViewFieldView extends Backbone.View
  ...
  render: ->
    @$el.addClass('response-field-' + @model.get(Formbuilder.options.mappings.FIELD_TYPE))
        .data('cid', @model.cid)
        .html(Formbuilder.templates["templates/formbuilder/view/base#{if !@model.is_input() then '_non_input' else ''}"]({rf: @model}))
...
class EditFieldView extends Backbone.View
  ...
  render: ->
    @$el.html(Formbuilder.templates["templates/formbuilder/edit/base#{if !@model.is_input() then '_non_input' else ''}"]({rf: @model}))
...
class BuilderView extends Backbone.View
  ...
  render: ->
    @$el.html Formbuilder.templates['templates/formbuilder/page']()
...
class Formbuilder
  ...
  @options:
    ...
    mappings:
      ...
      PREFILL_WITH: 'field_options.prefill_with'
